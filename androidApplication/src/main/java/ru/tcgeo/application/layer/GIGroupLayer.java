...
import ru.tcgeo.application.layer.requestor.GIDataRequestor;
...
public class GIGroupLayer extends GILayer
{
	...
	@Override
	public Observable<Tile> getRedrawTiles(final GIBounds area, final Rect viewRect, final float scaleFactor) {
...
//		Observable.combineLatest(Observable.just(m_list), new Func1<GITuple, Tile>() {
//
//            @Override
//            public Tile call(GITuple giTuple) {
//                return null;
//            }
//        })
		...
		return Observable.from(m_list)
				...
				.flatMap(new Func1<GITuple, Observable<Tile>>() {
					...
					public Observable<Tile> call(GITuple giTuple) {
						return giTuple.layer.getRedrawTiles(GIBounds.copy(area), viewRect, scaleFactor);
					}
				})
